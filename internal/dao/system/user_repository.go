...
// CreateUser 创建用户
func (u UserDao) CreateUser(dto system.UserDto) error {
...
func (u UserDao) QueryUserByUserId(userId int64) (*system2.User, error) {
	var sysUser system2.User
...
func (u UserDao) QueryUserByUsername(username string) (*system2.User, error) {
	var sysUser system2.User
...
func (u UserDao) QueryUserByUsernameOrMobile(account string) ([]system2.User, error) {
	var sysUser []system2.User

	err := u.db.Model(&system2.User{}).Where("user_name = ? or mobile = ?", account, account).Find(&sysUser).Error
...
func (u UserDao) QueryUserMenus(userId int64) ([]system2.Menu, error) {
	sql := `select u.*
from sys_user_role t
         left join sys_role usr on t.role_id = usr.id
         left join sys_role_menu srm on usr.id = srm.role_id
...
func (u UserDao) QueryUserList(userListDto system.QueryUserListDto) ([]system2.User, int64) {
...
// UpdateUser 更新用户
func (u UserDao) UpdateUser(dto system.UserDto) error {
...
func (u UserDao) DeleteUserByIds(ids []int64) error {
	return u.db.Where("id in (?)", ids).Delete(&system2.User{}).Error
}
