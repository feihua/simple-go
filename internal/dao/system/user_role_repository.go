...
func (u UserRoleDao) IsAdministrator(userId int64) bool {
	...
	var count int64
	// 1是预留超级管理员角色的id
	u.db.Model(&system.UserRole{}).Where("user_id= ? and role_id = 1", userId).Count(&count)
...
func (u UserRoleDao) UpdateUserRoleList(userId int64, userRoles []system.UserRole) error {

	// 先删除
	u.db.Where("user_id = ?", userId).Delete(&system.UserRole{})

	return u.db.Model(&system.UserRole{}).CreateInBatches(&userRoles, len(userRoles)).Error
}
